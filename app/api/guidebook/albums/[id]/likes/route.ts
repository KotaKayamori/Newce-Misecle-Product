...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (existing) {
      const { error } = await supabase.from("photo_album_likes").delete().eq("album_id", albumId).eq("user_id", user.id)
      if (error) return NextResponse.json({ error: '削除に失敗しました' }, { status: 500 })
      return NextResponse.json({ liked: false })
    } else {
      const { error } = await supabase.from("photo_album_likes").insert({ album_id: albumId, user_id: user.id, created_at: new Date().toISOString() })
...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { count, error: cErr } = await supabase.from("photo_album_likes").select("*", { count: 'exact', head: true }).eq("album_id", albumId)
    ...
    if (authHeader?.startsWith("Bearer ")) {
      const { data: { user } } = await supabase.auth.getUser()
      if (user) {
        const { data } = await supabase.from("photo_album_likes").select("id").eq("album_id", albumId).eq("user_id", user.id).single()
